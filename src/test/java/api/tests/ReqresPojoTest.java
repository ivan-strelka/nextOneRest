...
import api.pojo.colors.Data;
import api.pojo.registration.Register;
import api.pojo.registration.SuccessUserReg;
import api.pojo.registration.UnsuccessUserReg;
import api.pojo.user.UserData;
import api.pojo.user.UserTime;
import api.pojo.user.UserTimeResponse;
...
public class ReqresPojoTest {
    ...
    public void checkServerAndPcDateTest() {
        ...
        String currentTimeExp = Clock.systemUTC().instant().toString().replaceAll(regex11, "");

        System.out.println("currentTime -- >" + currentTime);
        System.out.println("currentTimeExp -- >" + currentTimeExp);
        System.out.println("response.getUpdatedAt -- >" + response.getUpdatedAt().replaceAll(regex, ""));

        Assert.assertEquals(response.getUpdatedAt().replaceAll(regex, ""), currentTimeExp);
    }
